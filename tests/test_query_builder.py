...
from aiomotorengine.query_builder.node import QCombination
...
class TestQueryBuilder(AsyncTestCase):
    ...
    async def create_test_users(self):
        ...
        self.user2 = await User.objects.create(
        ...
        self.user3 = await User.objects.create(
    ...
    async def test_can_query_using_q_with_is_null(self):
        ...
        users = await User.objects.filter(Q(nullable__is_null=False)).find_all()
        expect(users).to_length(2)
