...
class TestAggregation(AsyncTestCase):
    ...
    @async_test
    async def test_can_aggregate_with_sorting(self):
        result = await User.objects.aggregate.order_by(
    ...
    @async_test
    async def test_can_average_city_population_by_raw_query(self):
        results = await City.objects.aggregate.raw([
            {
                '$group': {
