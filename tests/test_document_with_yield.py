...
class TestDocument(AsyncTestCase):
    ...
    async def test_can_find_proper_document(self):
        await User.objects.create(email="heynemann@gmail.com", first_name="Bernardo", last_name="Heynemann")
        await User.objects.create(email="someone@gmail.com", first_name="Someone", last_name="Else")
    ...
    async def test_can_count_documents(self):
        ...
        user_count = await User.objects.filter(first_name="Bernardo").count()
    ...
    async def test_can_save_and_get_reference_without_lazy(self):
        ...
        comment = await CommentNotLazy(text="Comment text", user=user).save()
